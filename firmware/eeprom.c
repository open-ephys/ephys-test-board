...
int eeprom_init()
{
    ...
    // gpio_pull_up(MODULE_DETECT); // TODO: Remove for rev C where this is actually implemented
...
int eeprom_read_module(mode_context_t *ctx)
{
    if (gpio_get(MODULE_DETECT)) {
        ...
        int rc = eeprom_read(MODULE_I2C, EEPROM_ADDRESS, EEPROM_MAGIC_OFFSET, buf, EEPROM_NAME_OFFSET);
